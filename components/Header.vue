...
const clickReloadHandler = () => {
  ...
  emitter.emit("timeline:reload");
  // エラー解消はこれとかでできないかな
  // https://nuxt.com/docs/guide/going-further/hooks#additional-hooks
};
...
<template>
  <nav class="header">
    ...
    <SearchBar v-model:value="syncedSearchText" @clickDelete="clickSearchDeleteHandler" />

    <div class="header__settings">
      <div class="header__settingsReload" @click="clickReloadHandler">
        <svg width="24px" height="24px" xml:space="preserve" viewBox="0 0 512 512">
          <g>
            <path
		...
		C512.014,192.446,486.844,134.565,446.025,92.206z"
              style="fill: rgb(75, 75, 75)"
            ></path>
          </g>
        </svg>
      </div>
      <div class="header__settingsAccount">
        <img
          v-if="avatarUrl"
          class="header__settingsAccountIcon"
          :src="avatarUrl"
          alt="Avatar Image"
        />
      </div>
